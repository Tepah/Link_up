    ...
    <div role="none" class="grid grid-cols-7 gap-2 md:gap-4" on:mouseup={handleMouseUp}>
        ...
        {#each dates as {date, isSelected, hover, isPrevMonth, isNextMonth}, index}
            <button
                    ...
                    on:click={checkBeforeToday(new Date(year, month, date), isPrevMonth, isNextMonth) ? null : () => {
                      handleDateClick(index)}}
                    on:mousedown={checkBeforeToday(new Date(year, month, date), isPrevMonth, isNextMonth) ? null : () => {
                      handleMouseDown(index)}}
                    on:mouseenter={checkBeforeToday(new Date(year, month, date), isPrevMonth, isNextMonth) ? null : () => {
